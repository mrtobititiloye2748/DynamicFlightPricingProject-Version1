...
int get_valid_bool(const char* prompt) {
    ...
    do {
        printf("%s (type 1 for Yes, 0 for No): ", prompt);
...
double calculate_total_revenue(Passenger passengers[], int count) {
    double total = 0.0;

    for (int i = 0; i < count; i++) {

        double ticket = calculate_ticket_price(
            passengers[i].isWeekend,
            passengers[i].isTouristDestination,
            passengers[i].hasLoyaltyProgram
        );

        double baggage = online_checkin(
            passengers[i].totalWeight,
            passengers[i].totalBags
        );

        double seat = select_seat(
            passengers[i].wantsWindow,
            passengers[i].wantsFront,
            passengers[i].wantsAisle
        );

        if (ticket >= 0 && baggage >= 0 && seat >= 0) {
            total += ticket + baggage + seat;
        }
    }
